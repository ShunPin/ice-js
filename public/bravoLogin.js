(function(module, require, exports) {
    ...
    var BravoLogin = Ice.Class({
         ...
         */
        setLanguageTag: function(lang_tag) {
        ...
        registerAllFunctionalListener: function() {
            var promise = new Ice.Promise();
         ...
         * @return {boolean}
         */
        _registerFunctionalListener: function(proxy_name,
                                              callback,
                                              register_to_ice) {
                ...
                promise =
                    self.glacier.router.getCategoryForClient().then(function(category) {
                        ...
                        if( result.resultCode == RequestContract.RequestResult.ResultCode_Success ) {
                            ...
                            self._callconnectionLister(self.ClientFacadeCommand.AddCallbackSucceed, JSON.stringify({ ProxyName: proxy_name }));
                            ...
                            self._callconnectionLister(self.ClientFacadeCommand.InvokeError, JSON.stringify({ ProxyName: proxy_name }));
                        ...
                        self._callconnectionLister(self.ClientFacadeCommand.AddCallbackError, JSON.stringify({ ProxyName: proxy_name }));
         ...
         * @param {String} proxy_name
         */
        _unregisterFunctionalListener: function(proxy_name) {
                ...
                if( typeof callbackInfo.callbackPrx != 'undefined' ) {
                    invokablePrx.RemoveCallback(callbackPrx, self.glacier.session).then(
                        ...
                        function(ex) {
                            ...
                            // TODO: Test
                            debugger;
                            self._callconnectionLister(self.ClientFacadeCommand.RemoveCallbackError, JSON.stringify({ ProxyName: proxy_name }));
                        });
                }
            }
         ...
         * @private
         */
        _callconnectionLister: function(method, data) {
            this.connectionListener.forEach(function(listener) {
                listener(method, data);
            }, this);
        },

        _callFunctionalListener: function(proxy_name, method, result, data) {
         ...
         * @return {String}
         */
        _decryptRsaData: function(enc_data) {
         ...
         * @param {function} callback
         */
        _sendEncryptionRequest: function(url,
                                         headers,
                                         params,
                                         callback) {
            // AES 加密, 並轉 base64
